...
%% ----------------------------------------------------------------------------------------
\def\Version{\today { } V 2.05} % Modify error bar line plots
...
\hlc{We present the summarized results in this section accompanied by commentary referring to detailed results made available in Appendix {\ref{apx}}. Tables in this section and Appendix {\ref{apx}} use {\textcolor{dblue}{blue}} text to highlight prominent values. Plots accompany tables to assist in visualizing the comparative performance.}

We first present the overall category-wise averaged metrics and averaged standard deviations, followed by error-bar plots of the F-beta (0.5) validation score across the ten training-cum-testing rounds. Finally, we present performance of the ``super models'' -- best of the ten trained models, for each algorithm.
\begin{table*}[h]\centering
	...
	\begin{tabular}{@{}r l l@{}}
		...
		1 & Overall performance: Averaged over all 15 environments & Table \ref{tbl:ListEnvironments} \\
		2 & Simulated environment: Averaged over 3 noise setting variants & Table \ref{tbl:ListEnvironments} \\
		3 & Overall performance: Averaged over all 15 environments & Table \ref{tbl:ListEnvironments} \\
		4 & Overall performance: Averaged over all 15 environments & Table \ref{tbl:ListEnvironments} \\
		5 & Overall performance: Averaged over all 15 environments & Table \ref{tbl:ListEnvironments} \\
		\bottomrule
	\end{tabular}
...
Fig. \ref{fig:FbetaOverall} shows the error-bar plot of the overall F-beta (0.5) performance, across the 10 rounds of training. The blue line floating above the rest of the algorithms, for all metrics, is that of REINFORCE. The error-bars are also pretty small, indicating a comparatively small uncertainty. We notice that the other three algorithms are all centered closely around, roughly, 0.5.
\begin{figure}[h]
	\centering
...
\begin{figure}[!htbp]
	\begin{subfigure}{\textwidth}
		...
		\caption{Simulated environments.}
	\end{subfigure}
	
	\begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=\linewidth]{Singevariable_F05.pdf}  
		\caption{Univariate simple state environment.}
	\end{subfigure}
	
	\begin{subfigure}{\textwidth}
		\centering
		\includegraphics[width=\linewidth]{Multivariate_F05.pdf}  
		\caption{Multivariate complex state environment.}
	\end{subfigure}
	
	\caption{F-beta (0.5) performance across all categories.}
	\label{fig:FbetaPerformance}
\end{figure}
