...
.
Deep Q-Network \citep{DQN-mnih2013} significantly improved the earliest RL algorithm, Q-learning, by introducing neural networks to learn policies for high-dimension environments. \cite{DQN-mnih2013} introduced an ``experience replay buffer'' and a target network that was frozen and only periodically updated to significantly stabilize learning. $Q(s, a; \theta)$ is the function parameterized with $\theta$, that helps computes the Q values.
...
where $D$ is the replay memory and is sampled using a uniform distribution $U(D)$; $\theta^{-}$ represents parameters of the frozen target Q-network.

is a temporal-difference (TD) learning algorithm that 

\subsection{Advantage Actor Critic (A2C)}

A2C is a policy-gradient actor-critic algorithm \citep{A2C-mnih2016}. The policy-gradient family of algorithms strive to model and optimize the policy directly, while actor-critic structures consist of two network models -- a critic that updates function parameters $w$ of the value function (i.e either $Q_w(a \vert s)$ or $V_w(s)$); and a actor that updates the policy parameters  $\theta$ for $\pi_\theta(a \vert s)$, following the direction computed by critic.

A2C was proceeded by Asynchronous Advantage Actor Critic (A3C). A2C is a \textit{synchronous}, deterministic variant of A3C, and uses multiple computational workers to avoid the use of a replay buffer. 

\textbf{A2C}: Actors learn parameterized policy $\pi_{\theta}$ using the policy-gradient as shown in \ref{eq:A2C}. "\textcolor{red}{*SB3 says:*} 
