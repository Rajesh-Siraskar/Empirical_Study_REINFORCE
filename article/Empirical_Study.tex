...
Table \ref{tbl:ListEnvironments} summarizes the 15 environment variants and their three logical groups: (1) Simulated 1-3 (2) PHM univariate (4-12) and PHM multivariate (13-15).
...
\begin{table*}
	...
	\begin{tabular}{@{}r l rr@{}} \arrayrulecolor{black!40}\toprule
		...
		12 & PHM C06 SS (simple univariate - High noise & 1e-2 & 0.10 \\ \hdashline

		13 & PHM C01 MS (multivariate) - No noise & None & None \\
		14 & PHM C04 MS (multivariate) - Low noise & 1e-3 & 0.05 \\
		15 & PHM C06 MS (multivariate) - High noise & 1e-2 & 0.10 \\ \bottomrule
	\end{tabular}
	\caption{List of the fifteen environments}
...
The REINFORCE uses a \textit{single} internal layer of 64 units. The default set-up of all three algorithms by SB3 consists of \textit{two} layers of 64 neurons each. While we used ReLU (rectified linear unit) as the activation function, the other three algorithms used hyperbolic tangent (Tanh).

\subsection{Training}
Our training strategy ensured comparison of the algorithm performance, by ensuring the same environment variant, wear dataset, the noise and chance of breakdown parameters, are uniformly maintained across the four algorithms during a single training round. 

The REINFORCE was trained for 800 episodes for the simulated and PHM univariate variants, for all three noise and breakdown levels (none, low and high) -- Table \ref{tbl:ListEnvironments} items 1-12. For the PHM multivariate variant Table \ref{tbl:ListEnvironments} items 13-15, REINFORCE was trained for 1000 episodes. SB3 algorithms were trained for 10,000 episodes for all variants.
...
\begin{figure}[ht]
	...
	\begin{subfigure}{\textwidth}
		...
		\label{fig:tr-ms-f05}
	\end{subfigure}
	\caption{multivariate state environment -- Avg. performance over 10 rounds of model training}
	\label{fig:tr-ms-env}
\end{figure}
