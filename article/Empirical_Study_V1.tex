...
%% V.0.9: 23-07-23: Start Discussion. NOTE V.0.9 will be the long version with commented sections.
%% 					V.1.0 will be first release for external review
%%
%% 
%% ----------------------------------------------------------------------------------------
...
\begin{figure}[htp]	
	...
	\chronoevent[markdepth=-20pt]{1992}{REINFORCE}
...
\noindent\textbf{What does a single data point indicate?}: Consider the ``Simulated'' category -- 10 training rounds $\times$ 10 test rounds $\times$ 3 noise settings gives us 300 sample points. Note that a single test round is conducted with 40 randomly sampled wear datapoints and therefore 300 samples actually indicate 300 $\times$ 40 i.e. 12,000 samples from the wear dataset. Similarly, for the real-data univariate state -- 3 datasets $\times$ 10 training rounds $\times$ 10 test rounds $\times$ 3 noise settings gives us 900 sample points.
...
%\end{table*}					
\textbf{On the evaluation criteria:} It must be noted that the classification metrics we used to evaluate, assume that the human preventive maintenance policy is ideal and suggested based on experience. In practice this might not always be the case. The RL agent is trained to replace the tool optimally i.e. maximize use before reaching the threshold wear, and replace only when necessary so as to maintain lowest possible cost of replacement. In our dataset we assume the threshold to be ideally set and base the ``human'' action against that, rather than a real human action, which incidentally is not available for the PHM dataset anyway.
...
Implementing robust RL algorithms is complex and \cite{SB3-paper} mention how small implementation details can have a significant impact on performance that is often greater than the difference between the algorithms themselves. We hope our research will contribute to the empirical evidence in the study of this early policy-gradient algorithm with its non-OpenAI-Gym environment experiments as well as contribute in an infinitesimal way to enable AutoML for the field of PdM, using RL.
