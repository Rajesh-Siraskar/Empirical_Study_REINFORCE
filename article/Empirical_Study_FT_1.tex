...
\textcolor{red}{**KEEP MARJOV TOOL PROP HERE AND MOVE SOME OF THIS TO RL BASICS SECTION -- Formulating our problem to be solved by RL requires us to assume that the process satisfies the Markov property and is therefore a Markov Decision Process (MDP). Markov property implies that the transition of tool wear to another state is dependent only on the current state and not on any previous states. MDPs are defined by the tuple $<\mathcal{S, A, P, R, \gamma}>$. We will define the elements of state space $\mathcal{S}$, action space $\mathcal{A}$ and reward function $\mathcal{R}$ next. *}
